...
public class RNSentianceConverter {
  ...
  public static WritableMap convertUserContext(UserContext userContext) {
    ...
    for (Event event: userContext.getEvents()) {
      ...
      eventMap.putString("startTime", event.getStartTime().toString());
      if (event.getEndTime() != null) {
        eventMap.putString("endTime", event.getEndTime().toString());
      }

      eventMap.putString("type", event.getEventType().toString());

      if (event instanceof StationaryEvent) {
        addStationaryEventInfo(eventMap, (StationaryEvent) event);
      }
      else if (event instanceof TransportEvent) {
        addTransportEventInfo(eventMap, (TransportEvent) event);
      }
